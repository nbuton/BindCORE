    ...
    def _flush(pid, seq, lab):
        ...
        if "-" in raw:
            ...
            if debug:
                ...
                print(f"  [DEBUG] {pid}: label line has {n_dash} '-' characters "
        ...
        for lineno, line in enumerate(fh, 1):
            ...
            if line.startswith(">"):
                ...
                if re.search(r"[01\-]", line) and re.search(r"[A-Za-z]", line):
                    if debug:
                        print(f"  [DEBUG] line {lineno} for {current_id!r} looks mixed "
...
core_lip    = parse_prediction_file("data/predictions/core_lip_test.csv")
morfchibi   = parse_prediction_file("data/predictions/MoRFchibi_test.csv")

print(f"\n[parse_prediction_file] core_lip_test.csv   : {len(core_lip)} proteins")
...
for pid in common_ids:
    ...
    lc = len(core_lip[pid]["scores"])
    lm = len(morfchibi[pid]["scores"])
    if not (la == lc == lm):
        n_mismatch += 1
        print(f"  {pid}: annot={la}  core_lip={lc}  morfchibi={lm}")
if n_mismatch == 0:
...
_all_core_raw  = np.concatenate([core_lip[pid]["scores"]  for pid in common_ids])
...
for name, raw in [("CORE-LIP", _all_core_raw), ("MoRFchibi", _all_chibi_raw)]:
    ...
    print(f"  {name:10s}: min={raw.min():.4f}  max={raw.max():.4f}  "
...
for pid in common_ids:
    ...
    c_sc_raw  = core_lip[pid]["scores"]
    m_sc_raw  = morfchibi[pid]["scores"]
    c_bin     = core_lip[pid]["binary"]
    ...
    c_sc_raw      = c_sc_raw[:n]; m_sc_raw = m_sc_raw[:n]
    c_sc          = c_sc[:n];     m_sc     = m_sc[:n]
    c_bin         = c_bin[:n];    m_bin    = m_bin[:n]

    # Apply mask: keep only residues with known labels
    lab      = lab[mask]
    c_sc_raw = c_sc_raw[mask]; m_sc_raw = m_sc_raw[mask]
    c_sc     = c_sc[mask];     m_sc     = m_sc[mask]
    c_bin    = c_bin[mask];    m_bin    = m_bin[mask]
    ...
    for name, sc, bn in [("CORE_LIP", c_sc, c_bin), ("MoRFchibi", m_sc, m_bin)]:
...
print(f"Total residues: {len(y_true):,}  |  MoRF residues: {y_true.sum():,} ({100*y_true.mean():.1f}%)")
...
for mask_name, mask in [("CORE-LIP=1, MoRFchibi=0", (y_core_bin == 1) & (y_chibi_bin == 0)),
                         ("CORE-LIP=0, MoRFchibi=1", (y_core_bin == 0) & (y_chibi_bin == 1))]:
    if mask.sum() > 10:
        true_rate = y_true[mask].mean()
        print(f"    {mask_name}: {mask.sum():,} residues, true MoRF rate = {100*true_rate:.1f}%")
...
for ax, (name, y_score, y_bin) in zip(axes, [
        ("CORE-LIP",  y_core,  y_core_bin),
        ("MoRFchibi", y_chibi, y_chibi_bin)]):
...
sc  = axes[0].scatter(y_core[idx], y_chibi[idx], c=y_true[idx],
                       cmap="RdBu_r", alpha=0.4, s=5, vmin=0, vmax=1)
plt.colorbar(sc, ax=axes[0], label="True label")
axes[0].set_xlabel("CORE-LIP score"); axes[0].set_ylabel("MoRFchibi score")
...
axes[1].set_xlabel("CORE-LIP score"); axes[1].set_ylabel("MoRFchibi score")
...
for col_offset, (name, raw, norm, color) in enumerate(pairs):
    for row_i, (label_val, lbl) in enumerate([(1, "MoRF"), (0, "Non-MoRF")]):
        ...
        # Raw
        axes[row_i, col_offset*2].hist(raw[mask_lbl],  bins=60, color=color, alpha=0.7, edgecolor="none", density=True)
        ...
        # Normalised
        axes[row_i, col_offset*2+1].hist(norm[mask_lbl], bins=60, color=color, alpha=0.7, edgecolor="none", density=True)
...
plt.suptitle("Raw vs Rank-Normalised Score Distributions by True Label", y=1.01, fontsize=13, fontweight="bold")
plt.tight_layout()
plt.savefig("figures/04_score_distributions_raw_vs_norm.png", dpi=150, bbox_inches="tight")
plt.close()
...
for row_i, label_val, lbl in [(0, 1, "MoRF (label=1)"), (1, 0, "Non-MoRF (label=0)")]:
    mask_lbl = y_true == label_val
    for col_i, (name, y_score) in enumerate([("CORE-LIP", y_core), ("MoRFchibi", y_chibi)]):
        axes[row_i, col_i].hist(y_score[mask_lbl], bins=60, color=PALETTE[name], alpha=0.7,
...
plt.suptitle("Normalised Score Distributions by True Label", y=1.01, fontsize=13, fontweight="bold")
plt.tight_layout()
plt.savefig("figures/04_score_distributions.png", dpi=150, bbox_inches="tight")
plt.close()

# ── 6.5  Per-protein AUC comparison ──────────────────────────────────────────
valid = pp_df.dropna(subset=["CORE_LIP_AUC", "MoRFchibi_AUC"])
fig, axes = plt.subplots(1, 2, figsize=(13, 5))

axes[0].scatter(valid["CORE_LIP_AUC"], valid["MoRFchibi_AUC"],
                alpha=0.5, s=20, color="#555")
axes[0].plot([0,1],[0,1],"r--", lw=1)
axes[0].set_xlabel("CORE-LIP AUC"); axes[0].set_ylabel("MoRFchibi AUC")
axes[0].set_title("Per-protein AUC")

diff = valid["CORE_LIP_AUC"] - valid["MoRFchibi_AUC"]
...
axes[0].scatter(pp_df["CORE_LIP_MCC"], pp_df["MoRFchibi_MCC"],
                alpha=0.5, s=20, color="#555")
axes[0].plot([-1,1],[-1,1],"r--", lw=1)
axes[0].set_xlabel("CORE-LIP MCC"); axes[0].set_ylabel("MoRFchibi MCC")
axes[0].set_title("Per-protein MCC")

diff_mcc = pp_df["CORE_LIP_MCC"] - pp_df["MoRFchibi_MCC"]
...
ax.set_ylabel("Residue count"); ax.set_title("Prediction Agreement Breakdown")
for bar, val in zip(bars, counts):
    ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + len(y_true)*0.003,
            f"{val:,}\n({100*val/len(y_true):.1f}%)", ha="center", va="bottom", fontsize=10)
plt.tight_layout()
...
w = 0.35
for i, (model, color) in enumerate([("CORE-LIP", PALETTE["CORE-LIP"]),
                                      ("MoRFchibi", PALETTE["MoRFchibi"])]):
    vals = [metrics_df.loc[model, m] for m in metric_cols]
    bars = ax.bar(x + i*w - w/2, vals, w, label=model, color=color, alpha=0.85)
ax.set_xticks(x); ax.set_xticklabels(metric_cols, rotation=20, ha="right")
ax.set_ylim(0, 1.12); ax.axhline(0.5, color="grey", linestyle="--", lw=0.8)
ax.set_title("Aggregate Metric Comparison"); ax.legend()
plt.tight_layout()
...
for ax, (name, y_bin) in zip(axes, [("CORE-LIP", y_core_bin), ("MoRFchibi", y_chibi_bin)]):
...
fig, ax = plt.subplots(figsize=(7, 5))
ax.hist(pp_df["score_pearson"].dropna(), bins=40, color="#8E44AD", alpha=0.8,
        edgecolor="none", label="Pearson r")
ax.hist(pp_df["score_spearman"].dropna(), bins=40, color="#E67E22", alpha=0.6,
        edgecolor="none", label="Spearman ρ")
...
fig, axes = plt.subplots(1, 2, figsize=(13, 5))
for ax, name, col in [(axes[0], "CORE-LIP",  "CORE_LIP_AUC"),
...
for name, y_score, color in [("CORE-LIP", y_core, PALETTE["CORE-LIP"]),
                               ("MoRFchibi", y_chibi, PALETTE["MoRFchibi"])]:
    bin_idx = np.digitize(y_score, bins) - 1
    bin_idx = np.clip(bin_idx, 0, len(bins)-2)
    frac_pos = [y_true[bin_idx == i].mean() if (bin_idx == i).sum() > 0 else np.nan
                for i in range(len(bins)-1)]
    mids = (bins[:-1] + bins[1:]) / 2
    ax.plot(mids, frac_pos, "o-", color=color, lw=2, label=name)
ax.plot([0,1],[0,1],"k--", lw=1, label="Perfect calibration")
ax.set_xlabel("Mean predicted score"); ax.set_ylabel("Fraction of positives")
...
for pid in common_ids:
    ...
    length_records.append({
        ...
        "CORE_LIP_AUC":   row_pp["CORE_LIP_AUC"].values[0],
        "MoRFchibi_AUC":  row_pp["MoRFchibi_AUC"].values[0],
        "CORE_LIP_AUPRC": row_pp["CORE_LIP_AUPRC"].values[0],
        "MoRFchibi_AUPRC":row_pp["MoRFchibi_AUPRC"].values[0],
        "CORE_LIP_MCC":   row_pp["CORE_LIP_MCC"].values[0],
...
thresholds = np.unique(np.percentile(len_df["length"], np.linspace(5, 100, 40)).astype(int))
...
for thresh in thresholds:
    ...
    cum_results.append({
        ...
        "CORE_LIP_AUPRC":  average_precision_score(labs_pool, core_pool),
        "MoRFchibi_AUPRC": average_precision_score(labs_pool, chibi_pool),
        "CORE_LIP_AUC":    roc_auc_score(labs_pool, core_pool),
...
for bin_label, grp in len_df.groupby("length_bin", observed=True):
    ...
    if len(np.unique(labs_pool)) >= 2:
        row_b["CORE_LIP_AUPRC"]  = average_precision_score(labs_pool, core_pool)
        row_b["MoRFchibi_AUPRC"] = average_precision_score(labs_pool, chibi_pool)
        row_b["CORE_LIP_AUC"]    = roc_auc_score(labs_pool, core_pool)
        row_b["MoRFchibi_AUC"]   = roc_auc_score(labs_pool, chibi_pool)
        row_b["CORE_LIP_MCC_mean"]   = grp["CORE_LIP_MCC"].mean()
        row_b["MoRFchibi_MCC_mean"]  = grp["MoRFchibi_MCC"].mean()
    else:
        for k in ["CORE_LIP_AUPRC","MoRFchibi_AUPRC","CORE_LIP_AUC","MoRFchibi_AUC",
                  "CORE_LIP_MCC_mean","MoRFchibi_MCC_mean"]:
            row_b[k] = np.nan
    bin_results.append(row_b)
bin_df = pd.DataFrame(bin_results).dropna(subset=["CORE_LIP_AUPRC"])

print(f"\n[Length-stratified analysis]  {len(cum_df)} cumulative thresholds,  {len(bin_df)} bins")
print(bin_df[["bin","n_proteins","n_residues","CORE_LIP_AUPRC","MoRFchibi_AUPRC",
              "CORE_LIP_AUC","MoRFchibi_AUC"]].round(3).to_string(index=False))

# ── Figure 13: Cumulative AUPRC/AUC up to length L ──────────────────────────
fig, axes = plt.subplots(1, 2, figsize=(14, 5))

ax = axes[0]
ax.plot(cum_df["max_length"], cum_df["CORE_LIP_AUPRC"],
        color=PALETTE["CORE-LIP"],  lw=2, marker="o", ms=4, label="CORE-LIP")
ax.plot(cum_df["max_length"], cum_df["MoRFchibi_AUPRC"],
        color=PALETTE["MoRFchibi"], lw=2, marker="s", ms=4, label="MoRFchibi")
...
for _, r in cum_df.iloc[::8].iterrows():
    ax.annotate(f"n={int(r['n_proteins'])}", (r["max_length"], r["CORE_LIP_AUPRC"]),
                textcoords="offset points", xytext=(0, 8), fontsize=7, ha="center", color="grey")

ax = axes[1]
ax.plot(cum_df["max_length"], cum_df["CORE_LIP_AUC"],
        color=PALETTE["CORE-LIP"],  lw=2, marker="o", ms=4, label="CORE-LIP")
ax.plot(cum_df["max_length"], cum_df["MoRFchibi_AUC"],
        color=PALETTE["MoRFchibi"], lw=2, marker="s", ms=4, label="MoRFchibi")
ax.set_xlabel("Max sequence length (proteins \u2264 L)")
...
plt.suptitle("Performance on Proteins UP TO a Given Length", fontsize=13, fontweight="bold")
plt.tight_layout()
plt.savefig("figures/13_cumulative_performance_vs_length.png", dpi=150, bbox_inches="tight")
plt.close()
...
w = 0.35
tick_labels = [f"{int(r['length_median'])} aa\n(n={int(r['n_proteins'])})"
...
for ax, metric, ylabel, title in [
    ...
    c_col = f"CORE_LIP_{metric}"
    m_col = f"MoRFchibi_{metric}"
    bars1 = ax.bar(x - w/2, bin_df[c_col], w, label="CORE-LIP",
                   color=PALETTE["CORE-LIP"],  alpha=0.85, edgecolor="white")
    bars2 = ax.bar(x + w/2, bin_df[m_col], w, label="MoRFchibi",
                   color=PALETTE["MoRFchibi"], alpha=0.85, edgecolor="white")
    ax.set_xticks(x)
...
plt.suptitle("Model Performance Evolution Across Sequence Length Bins", fontsize=13, fontweight="bold")
plt.tight_layout()
...
fig, axes = plt.subplots(1, 2, figsize=(14, 5))
valid_auprc = pp_df.dropna(subset=["CORE_LIP_AUPRC", "MoRFchibi_AUPRC"])

for ax, name, col in [(axes[0], "CORE-LIP",  "CORE_LIP_AUPRC"),
                       (axes[1], "MoRFchibi", "MoRFchibi_AUPRC")]:
    sc = ax.scatter(valid_auprc["length"], valid_auprc[col],
                    c=valid_auprc["morf_fraction"], cmap="RdYlGn",
                    alpha=0.7, s=40, edgecolors="none", vmin=0, vmax=1)
...
plt.suptitle("Per-protein AUPRC vs Sequence Length (colour = MoRF fraction)",
...
print(metrics_df[["AUC-ROC","AUPRC","MCC","F1","Precision","Recall","Balanced_Acc"]].round(4).to_string())
...
has_auc  = pp_df["CORE_LIP_AUC"].notna() & pp_df["MoRFchibi_AUC"].notna()
no_auc   = ~has_auc
n_auc    = has_auc.sum()
n_no_auc = no_auc.sum()

core_better_auc  = (pp_df.loc[has_auc,  "CORE_LIP_AUC"]  > pp_df.loc[has_auc,  "MoRFchibi_AUC"]).sum()
chibi_better_auc = (pp_df.loc[has_auc,  "MoRFchibi_AUC"] > pp_df.loc[has_auc,  "CORE_LIP_AUC"]).sum()
core_better_mcc  = (pp_df.loc[no_auc,   "CORE_LIP_MCC"]  > pp_df.loc[no_auc,   "MoRFchibi_MCC"]).sum()
chibi_better_mcc = (pp_df.loc[no_auc,   "MoRFchibi_MCC"] > pp_df.loc[no_auc,   "CORE_LIP_MCC"]).sum()
...
print(f"  Metric used: AUC on {n_auc} proteins, MCC on {n_no_auc} proteins (single-class after masking)")
print(f"  CORE-LIP  better : {core_better_total:>3}/{len(pp_df)}  ({100*core_better_total/len(pp_df):.1f}%)  "
      f"[AUC: {core_better_auc}, MCC: {core_better_mcc}]")
print(f"  MoRFchibi better : {chibi_better_total:>3}/{len(pp_df)}  ({100*chibi_better_total/len(pp_df):.1f}%)  "
      f"[AUC: {chibi_better_auc}, MCC: {chibi_better_mcc}]")
print(f"  Tie              : {tie_total:>3}/{len(pp_df)}  ({100*tie_total/len(pp_df):.1f}%)")
...
for name, y_bin in [("CORE-LIP", y_core_bin), ("MoRFchibi", y_chibi_bin)]:
    print(f"  {name}: predicted positives = {100*y_bin.mean():.1f}%  "
          f"(true positive rate = {y_true.mean()*100:.1f}%)")

print(f"\n[5] Per-protein AUC stats")
for name, col in [("CORE-LIP","CORE_LIP_AUC"), ("MoRFchibi","MoRFchibi_AUC")]:
    vals = pp_df[col].dropna()
    print(f"  {name}:  mean={vals.mean():.3f}  median={vals.median():.3f}  std={vals.std():.3f}  "
          f"min={vals.min():.3f}  max={vals.max():.3f}")

print(f"\n[6] Calibration (Brier score – lower is better)")
for name in ["CORE-LIP", "MoRFchibi"]:
    print(f"  {name}: Brier = {metrics_df.loc[name,'Brier']:.4f}  "
          f"Log-loss = {metrics_df.loc[name,'LogLoss']:.4f}")


print(f"\n[7] Performance vs Sequence Length (binned)")
print(bin_df[["bin","n_proteins","CORE_LIP_AUPRC","MoRFchibi_AUPRC","CORE_LIP_AUC","MoRFchibi_AUC"]].round(3).to_string(index=False))
...
for kl in key_lengths:
    row_c = cum_df[cum_df["max_length"] >= kl].iloc[0] if (cum_df["max_length"] >= kl).any() else None
    if row_c is not None:
        print(f"  <= {kl:4d} aa ({int(row_c['n_proteins']):3d} proteins): "
              f"CORE-LIP AUPRC={row_c['CORE_LIP_AUPRC']:.3f}  MoRFchibi AUPRC={row_c['MoRFchibi_AUPRC']:.3f}")

print("\nAll figures saved to ./figures/")
print("Tables: aggregate_metrics.csv | per_protein_metrics.csv | "
